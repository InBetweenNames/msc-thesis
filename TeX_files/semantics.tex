...
the triples above belong to \texttt{event1}.
...
To support prepositions such as ``in'', which could refer to either a location or a point in time, we had to introduce a new definition
for prepositions in our semantics.  First, some new terminology:

\begin{definition}[Property]
	A \texttt{predicate} of a triple in an event-based triplestore.
\end{definition}

For example, in the triple \texttt{(event1000, with\_implement, refractor\_telescope\_1)}, \texttt{with\_implement} is a property.  Since the triple belongs to \texttt{event1000}, we say that \texttt{with\_implement} is a property of \texttt{event1000}.

In general, there is no restriction on the number of entities in an event that share the same property.
...
For example, the \texttt{subjects} or \texttt{objects} of an event are the entities with property \texttt{subject} or \texttt{object} of that event, respectively.
...
We also define the {\em ENTEVPROP} relation.  Informally, {\em ENTEVPROP}$(evs, prop)$ is the entity-event relation defined by the set of events {\em evs} under the property {\em prop}, with {\em prop} being ``pulled out'' of the event list {\em evs}, forming a relation between the properties and events.

\begin{definition}[ENTEVPROP(evs, prop) relation]
	\[\text{ENTEVPROP(evs, prop) = }\{\text{(ent, ev) : ent } \in \text{ getts(ev, prop, ANY) \& ev} \in \text{evs} \}\]
\end{definition}

In addition to this, we define the {\em ENTEVTYPE} relation.  Informally, this behaves like the ENTEVPROP relation except it accepts an event type as an argument
rather than a set of events, obtaining the set of events by gathering all triples of type $ev_type$ using the \texttt{getts} function.

\begin{definition}[ENTEVTYPE(ev\_type, prop) relation]
	\begin{equation}
	\begin{split}
	\text{ENTEVTYPE(ev\_type, prop) = ENTEVPROP(evs, prop)} \\ \text{where evs = getts(ANY, type, ev\_type)}
	\end{split}
	\end{equation}
\end{definition}
