...
\begin{definition}[Event]
	A set of meaningfully connected physical or abstract phenomena
\end{definition}
...
Functions in Haskell that take two arguments may be treated as binary operators by surrounding the function name with backticks (\texttt{`}) in a function call.  For example, if the function \texttt{plus} takes two numeric arguments, then the Haskell expressions ``\texttt{plus 1 2}'' and ``\texttt{1 `plus` 2}'' are equivalent.
...
A {\em termphrase} was defined as being either a proper noun or a determiner phrase\cite{frost2014denotational}.
...
y_n)$ in the argument of \texttt{collect} are grouped into one pair $(x, \left\{y_1, y_2, ..., y_n\right\})$ in the returned binary relation.
...
$R$'s arguments (if any) are substituted into the set definition on the right hand side of the equation.
We may combine this syntax with function currying, as used in the denotations for English words in our semantics,
to simplify our definitions.-
...
This happens because in the original \texttt{filter\_ev} function, the sets of events were not actually ``honed down'' before being passed to subsequent predicates in the preposition chain.
...
\begin{figure}
	\centering
	\fbox{\includegraphics[width=\textwidth]{eventfiltering}}
...
with existing nomenclature in mathematics.  Third, we overcame two shortcomings with how prepositions were originally defined in EV-FLMS.  Fourth, we unified
